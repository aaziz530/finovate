...
public class ForumService {
    ...
    // UPDATE
    public boolean updateForum(Forum forum, Long userId, User.Role role, boolean isBlocked) throws SQLException {
        if (isBlocked) throw new SecurityException("Utilisateur bloqué");

        Forum existing = getForumById(forum.getId());
        if (existing == null) return false;

        if (role == User.Role.USER && !existing.getIdCreator().equals(userId))
    ...
    // DELETE
    public boolean deleteForum(Long forumId, Long userId, User.Role role) throws SQLException {
        Forum forum = getForumById(forumId);
        if (forum == null) return false;

        if (role == User.Role.USER && !forum.getIdCreator().equals(userId))
