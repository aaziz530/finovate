...
import org.example.services.IGoalService;
...
public class GoalsController implements Initializable {
    ...
    private void refreshDashboard() {
        try {
            float balance = goalService.getCurrentBalance(SessionManager.getCurrentUserId());
            balanceLabel.setText(String.format("%.2f TND", balance));

            List<Goal> goals = goalService.getGoalsByUserId(SessionManager.getCurrentUserId());
    ...
    private void handleAddGoal() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/goal-dialog.fxml"));
            ...
            if (controller.isSaveClicked() && controller.getGoal() != null) {
                ...
                newGoal.setIdUser(SessionManager.getCurrentUserId());
    ...
    private void handleAddFunds(Goal goal) {
        ...
        dialog.showAndWait().ifPresent(amountStr -> {
            try {
                ...
                goalService.addFundsToGoal(SessionManager.getCurrentUserId(), goal.getId(), amount);
                refreshDashboard();
