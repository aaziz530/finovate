...
public class AdminDashboardController implements Initializable {
    ...
    private void setupStatistics() {
        try {
            List<User> allUsers = userService.getAllUsers(User.Role.ADMIN);
    ...
    private void loadUsers() {
        try {
            List<User> users = userService.getAllUsers(User.Role.ADMIN);
            usersList.clear();
    ...
    private void populateUpdateDialog(User user) {
        ...
        if (updateRoleComboBox != null) {
            ...
            updateRoleComboBox.setValue(user.getRole().name());
        }
    }
    ...
    private void handleUpdateSave() {
        ...
        try {
            ...
            selectedUserForUpdate.setRole(User.Role.valueOf(updateRoleComboBox.getValue()));
            ...
            boolean updated = userService.updateUser(selectedUserForUpdate, 
                SessionManager.getCurrentUserId(), User.Role.ADMIN);
            ...
            if (updated) {
                ...
                showUpdateError("Failed to update user");
            }
    ...
    private void handleUnblockUser(User user) {
        try {
            userService.unblockUser(user.getId(), User.Role.ADMIN);
            loadUsers();
    ...
    private void handleDeleteUser(User user) {
        ...
        confirm.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    userService.deleteUser(user.getId(), User.Role.ADMIN);
                    loadUsers();
