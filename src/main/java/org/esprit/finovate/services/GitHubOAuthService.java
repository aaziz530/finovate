...
public class GitHubOAuthService {
    ...
    public GitHubUserInfo authenticate() throws Exception {
        ...
        server.createContext("/callback", exchange -> {
            try {
                ...
                if (params.containsKey("error")) {
                    responseHtml = "<html><body><h3>GitHub login failed</h3><p>You can close this window and return to the app.</p></body></html>";
                } else {
        ...
        try {
            String authUrl = buildAuthUrl(config.getClientId(), redirectUri, state);
            openBrowser(authUrl);

            Map<String, String> params = callbackParamsQueue.take();

            if (params.containsKey("error")) {
                throw new IllegalStateException("GitHub authentication error: " + params.get("error"));
            }
    ...
    private GitHubUserInfo fetchUserInfo(String accessToken) throws Exception {
        // Get user profile
        HttpRequest request = HttpRequest.newBuilder()
        ...
        String name = json.hasNonNull("name") ? json.get("name").asText() : null;
        String login = json.hasNonNull("login") ? json.get("login").asText() : null;

        // If email is null in profile, fetch from emails endpoint
        if (email == null || email.isBlank()) {
            email = fetchPrimaryEmail(accessToken);
        }

        if (email == null || email.isBlank()) {
            throw new IllegalStateException("GitHub did not return an email");
        }
        ...
        if (name != null && !name.isBlank()) {
            ...
            // Use login as first name if no name available
            firstName = login;
        }

        return new GitHubUserInfo(email, firstName, lastName);
    }
    ...
    private String fetchPrimaryEmail(String accessToken) throws Exception {
        ...
        if (emails.isArray()) {
            for (JsonNode emailNode : emails) {
                if (emailNode.hasNonNull("primary") && emailNode.get("primary").asBoolean()
                        && emailNode.hasNonNull("email")) {
                    return emailNode.get("email").asText();
                }
            }
            // If no primary, return first verified email
            for (JsonNode emailNode : emails) {
                if (emailNode.hasNonNull("verified") && emailNode.get("verified").asBoolean()
                        && emailNode.hasNonNull("email")) {
                    return emailNode.get("email").asText();
                }
            }
    ...
    public record GitHubUserInfo(String email, String firstName, String lastName) {
    }
}
